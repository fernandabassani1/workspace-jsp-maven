...
public class FuncionarioRepository {
	...
	String sql_exist_func = "SELECT COUNT(*) as count from funcionario f where f.email = ? and f.senha = ?";
	String sql_findByEmailAndSenha = "SELECT * from funcionario f WHERE f.email = ? and f.senha = ";
	...
	public Funcionario findByEmailAndSenha(String email, String senha){
		...
		try {
			pstm = db.conectar().prepareStatement(sql_findByEmailAndSenha);
	...
	public int countFuncionario(String email, String senha){
		PreparedStatement pstm;
		ResultSet rs;
		...
		try {
			pstm = db.conectar().prepareStatement(sql_exist_func);
			pstm.setString(1, email);
			pstm.setString(2, senha);
			rs = pstm.executeQuery();
			...
			while (rs.next()) {
				funcionario = new Funcionario();
				funcionario.setCount(rs.getInt("count"));
			}
